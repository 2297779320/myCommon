...
int main(void) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        async_req_ids[i] = async_request_submit(queue, data,
    ...
    for (int i = 0; i < 3; i++) {
        ...
        request_id_t req_id = async_request_submit_sync(queue, data,
                                                         NULL,  // 无上下文
                                                         -1,    // 无超时
                                                         &status);
    ...
    for (int i = 0; i < 3; i++) {
        printf("  Request %d: ThreadId=%d, Completed=%d, Failed=%d, Cancelled=%d\n",
               i, contexts[i].thread_id, contexts[i].completed_count,
